...
export function ResetPassword() {
  ...
  // CAPTCHA — a final human-verification gate before the new password is
  // saved. Supabase's updateUser() call doesn't take a captchaToken (that's
  // only for endpoints that send an email/OTP), so this is enforced on the
  // client: the widget must be solved before the form can be submitted.
  ...
  useEffect(() => {
    // Clicking the "reset password" link in the email brings the user back
    // here with a recovery token in the URL. Supabase's client auto-detects
    // it and fires this event once the temporary recovery session is set.
    ...
    // If nothing confirms a valid recovery session shortly after load, the
